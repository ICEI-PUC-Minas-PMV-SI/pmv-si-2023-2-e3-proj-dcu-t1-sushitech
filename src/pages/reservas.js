...
const Reservas = () => {
  ...
  const areas = [
    { name: 'Interna', color: '#c95000' }, // Cor azul para Interna
    { name: 'Externa', color: '#c95000' }, // Cor verde para Externa
    { name: 'Indiferente', color: '#c95000' }, // Cor laranja para Indiferente
  ];
    ...
    if (page === 'form') {
        ...
        <div className="modal-content">
          ...
          <div id="areas" className="areas-container">
            {areas.map((area) => (
             <div
             key={area.name}
             className={`area ${areasSelecionadas.includes(area.name) ? 'selected' : ''}`}
             style={{ backgroundColor: areasSelecionadas.includes(area.name) ? area.color : '' }}
             data-area={area.name}
          ...
          <br/>
          <button
            className={`confirmar-area-button ${areasSelecionadas ? 'enabled' : ''}`}
            onClick={confirmarAreas}
            disabled={!areasSelecionadas}
          >
            Confirmar Área
          </button>
        </div>
      ...
      return (
        <div className="modal-content">
          ...
          <button className="confirmar-reserva-button" onClick={confirmarReserva}>
            Confirmar Reserva
          </button>
        </div>
      );
  ...
  return (
    <div>
      <h1 className="title" >RESERVE JÁ!</h1>
      <div className="custom-layout">
        <div className="left-column">
          ...
          <button className="open-modal-button" onClick={openModal}>Fazer uma reserva</button>
          {isModalOpen && (
        ...
        <img src="/sushi-tech-2.jpg" alt="Sushi Tech" />
        </div>
    </div>
    </div>
  );
